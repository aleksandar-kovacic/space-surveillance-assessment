...
acpl_steps = 10
df = pd.read_csv("sma_inc_acpl/oldSSN.csv")
#df=pd.read_csv("../ARES_results/risk_reduction_90%/new_ssn_2016_RR90%.csv")
...
while i < int(len(df)/acpl_steps):
    ...
    d_ref_list.append(df_new.at[start, "d_ref"])
...
df_total_output = pd.DataFrame(
        data = {"f_risk_red": f_risk_red_list, "man_rate": man_rate_list, "sma": sma_list, "d_ref": d_ref_list}
    )
print(df_total_output)


df_total_output = df_total_output.pivot("d_ref", "sma", "man_rate")
ax = plt.axes()
...
"for inclination = 90°, radar wavelength = 0.15m and covariance " + "\n" + " matrix adjusted to expected Space Fence performance"})
ax.set_title("Manoeuvre Rate of SWARM B type mission dependent on the reference " + "\n" + " diameter d_ref with Space Fence for the Reference Epoch 2016")
ax.set_ylabel("Reference Diameter [m]")
