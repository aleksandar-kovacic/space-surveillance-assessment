...
df = pd.read_csv("SWARMA_oldSSN_2022.csv")
df1 = pd.read_csv("SWARMA_newSSN_2022.csv")
df2 = pd.read_csv("SWARMA_oldSSN_pop_2022.csv")
df3 = pd.read_csv("SWARMA_newSSN_pop_2022.csv")
satellite_name = "SWARM A"
ACPL = 0.0001
...
ax1.axhline(y = ax_y[0], color = "red")

ax1_line_x = np.array([1e-9, 1e-1])
ax1_line_y = np.array([ax_y[0], ax_y[0]])
ax1_x, ax1_y = intersection(df1["acpl"], df1["f_rem_risk"], ax1_line_x, ax1_line_y)
ax1.axvline(x = ax1_x[0]-0.0009, color = "red")
ax1.plot(ax1_x[0]-0.0009, ax1_y[0], marker='o', markersize=3, color="black")
...
ax2.set_title("Old SSN with population size threshold > 1.8 cm")
...
ax3.set_title("New SSN with population size threshold > 1.8 cm")
...
#0.08
fig.suptitle("SWARM A\nEpoch: 2022")
fig.tight_layout(pad = 3)
fig.savefig("SWARMA_rem_risk_analysis.pdf", dpi = 3000, bbox_inches = "tight")
plt.show()
