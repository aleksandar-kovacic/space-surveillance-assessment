...
while i < int(len(df)/acpl_steps):
    ...
    radar_wavelength_list.append(df_new.at[start, "radar_wavelength"])
...
df_total_output = pd.DataFrame(
        data = {"f_risk_red": f_risk_red_list, "man_rate": man_rate_list, "sma": sma_list, "radar_wavelength": radar_wavelength_list}
    )
...
ax = sns.heatmap(df_total_output, annot=True, fmt = ".0f", cbar_kws={'label': 'Manoeuvre rate at fractional risk reduction = 90% and i = 90°'})
ax.set_title("Manoeuvre rate of Sentinel-3A type mission with" + "\n" + "new SSN for the epoch 2022")
ax.set_ylabel("Radar wavelength $\u03BB$ [m]")
